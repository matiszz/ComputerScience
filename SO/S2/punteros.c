...
int main(int argc, char *argv[])
{
	...
	sprintf(buff, "Puntero %x buff_int=%x &buff_int[0]=%x\n", pbuff_int, buff_int, &buff_int[0]);
	...
	sprintf(buff, "Puntero sin inicializar pb=%x\n", pb);
	write( 1, buff, strlen(buff) );
	sprintf(buff, "Apuntamos a una zona segura pb=&b \n");
	write( 1, buff, strlen(buff) );
	pb = &b;
	b = 'H';
	sprintf(buff, "Puntero inicializado pb=%x, &b=%x b=%c *pb=%c\n", pb, &b, b, *pb);
	write( 1, buff, strlen(buff) );
	...
	for (i = 0; i < SIZE_BUFF; i++) {
		sprintf(buff, "Direccion %d de buff de enteros %x\n", i, pbuff_int);
		write( 1, buff, strlen(buff) );
		pbuff_int++;
	}
	sprintf(buff, "CHARS \n");
	write( 1, buff, strlen(buff) );
	for (i = 0; i < SIZE_BUFF; i++) {
		sprintf(buff, "Direccion %d de buff de caracteres %x\n", i, pb);
		write( 1, buff, strlen(buff) );
		pb++;
	}
