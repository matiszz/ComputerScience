...
init(Id, Master, Jitter) ->
    receive
        {peers, Nodes} ->
            server(Master, seq:new(Id), seq:new(Id), Nodes, [], [], Jitter, 0, 0)
    end.

server(Master, MaxPrp, MaxAgr, Nodes, Cast, Queue, Jitter, CounterMulti, CounterUni) ->
receive
    {send, Msg} ->
        Ref = make_ref(),
        request(Ref, Msg, Nodes, Jitter),
        NewCast = cast(Ref, Nodes, Cast),
        server(Master, MaxPrp, MaxAgr, Nodes, NewCast, Queue, Jitter, CounterMulti+1, CounterUni);
    {request, From, Ref, Msg} ->
        NewMaxPrpTemp = seq:maxfirst(MaxPrp,MaxAgr), %%actualizamos de forma diferente ya que tienes que quedarte con el identificador del proceso que lo ha enviado
        NewMaxPrp = seq:increment(NewMaxPrpTemp),
        From ! {proposal, Ref, NewMaxPrp},
        NewQueue = insert(Ref, Msg, NewMaxPrp, Queue),
        server(Master, NewMaxPrp, MaxAgr, Nodes, Cast, NewQueue, Jitter, CounterMulti, CounterUni+1);
    {proposal, Ref, Proposal} ->
        case proposal(Ref, Proposal, Cast) of
            {agreed, MaxSeq, NewCast} ->
                agree(Ref, MaxSeq, Nodes),
                server(Master, MaxPrp, MaxSeq, Nodes, NewCast, Queue, Jitter, CounterMulti, CounterUni+1);
            NewCast ->
                server(Master, MaxPrp, MaxAgr, Nodes, NewCast, Queue, Jitter, CounterMulti, CounterUni+1)
        end;
    {agreed, Ref, Seq} ->
        NewQueue = update(Ref , Seq, Queue),
        {AgrMsg, NewerQueue} = agreed(NewQueue),
        deliver(Master, AgrMsg),
        NewMaxAgr = seq:max(MaxAgr,Seq), %%Max entre el que tenias y seq
        server(Master, MaxPrp, NewMaxAgr, Nodes, Cast, NewerQueue, Jitter, CounterMulti, CounterUni+1);
    stop ->
        io:format("*************** CounterMulti = ~w; CounterUni = ~w ***************", [CounterMulti, CounterUni]),
        ok
end.
