...
void MyGLWidget::inicialitzaLlums() {
  // Aqui declarem el nostre vector. Despres hem de passar aquest vector al canal loquesiguiLoc
  // per tal que arribi al uniform. Per tant, indiquem el canal, el numero de vectors, i l'adreça inicial
  // del vector.

  sco = false; // Al principi volem veure la llum com SCA
  ences = true;

  colFocus = glm::vec3(0.9, 0.9, 0.9);
  ...
  // El posFocus el tractem a viewTransform(), perquè si volem SCA, s'ha de multiplicar per ViewMatrix
}
...
void MyGLWidget::createBuffers () {
  ...
  glm::vec3 amb(0.2,0.2,0.6);
  glm::vec3 diff(0.3,0.3,1.0);
  glm::vec3 spec(1,1,1);
  float shin = 200;
...
void MyGLWidget::carregaShaders() {
  ...
  // La forma de passar al Uniform un valor, es a través d'un canal. A aquest canal li diem loquesiguiLoc.
  // El que indiquem, es que quan el colFocus del Vertex Shader vulgui una variable, la busqui a colFocusLoc.
...
void MyGLWidget::viewTransform () {
  ...
  if (sco) posFocusSCO = glm::vec4(0.0, 0.0, 0.0, 1.0); // SCObservador
  else posFocusSCO = View * glm::vec4(posFocus, 1.0); // SCEscena
  glUniform4fv(posFocusLoc, 1, &posFocusSCO[0]);

  if (ences) colFocus = glm::vec3(0.9, 0.9, 0.9);
