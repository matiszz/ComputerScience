...
void Cjt_estudiants::recalcular_posicio_imax() {
	...
	for (int i = 0; i < nest; ++i) {
		if (vest[i].te_nota()){
			if (vest[i].consultar_nota() > notaA) {
				notaA = vest[i].consultar_nota();
				pos = i;
			}
			else if (vest[i].consultar_nota() == notaA) {
...
void Cjt_estudiants::esborrar_estudiant(int dni) {
	...
	while (not trobat) {
		...
		++i;
	} // Tenim i que es la posició del estudiant
	if (i-1 == imax) recalcular = true;
